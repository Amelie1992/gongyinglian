...
public class MarketTenderRecordServiceImpl implements MarketTenderRecordService {
    ...
    public void save(MarketTenderRecordEntity marketTenderRecord) {
        ...
        if (companyId != null) {
            ...
            marketTenderRecord.setCreateBy(companyEntity.getUserId());
        }
        marketTenderRecordDao.save(marketTenderRecord);

        // 保存附件
        MarketTenderAnnexEntity file = new MarketTenderAnnexEntity() {{
        ...
        if (StringUtils.isBlank(file.getFileName())){
            file.setFileName(SplitUtil.splitPicName(file.getUrl()));
        }
        marketTenderAnnexDao.save(file);
    }
