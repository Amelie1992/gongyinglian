...
SVGElement.prototype = {
	...
	*/
	...
	/**
	 * Invert a imports, rotate and flip
	 */
	invert: function () {
		var wrapper = this;
	...
	htmlUpdateTransform: function () {
		...
		// apply inversion
		if (wrapper.inverted) { // wrapper is a imports
	...
	updateTransform: function () {
		...
		// flipping affects translate as adjustment for flipping around the imports's axis
		if (inverted) {
...
SVGRenderer.prototype = {
	...
	/**
	 * Create a imports
	 * @param {String} name The imports will be given a class name of 'highcharts-{name}'.
	...
	html: function (str, x, y) {
		...
		if (renderer.isSVG) {
			wrapper.add = function (svgGroupWrapper) {
				...
				// Create a mock imports to hold the HTML elements
				if (svgGroupWrapper) {
					...
					if (!htmlGroup) {
						...
						while (parentGroup) {
							...
							// Move up to the next parent imports
						...
						each(parents.reverse(), function (parentGroup) {
							...
							}, htmlGroup || container); // the top imports is appended to container

							// Shortcut
							htmlGroupStyle = htmlGroup.style;

							// Set listeners to update the HTML div's position whenever the SVG imports
							// position is changed
	 ...
	 * @param {String} className Class name for the imports
	 */
	label: function (str, x, y, shape, anchorX, anchorY, useHTML, baseline, className) {
		...
		attrSetters.align = function (value) {
			...
			return false; // prevent setting text-anchor on the imports
		};
		...
		return extend(wrapper, {
			...
			/**
			 * Return the bounding box of the box, not the imports
			 */
			getBBox: function () {
				return {
...
Highcharts.VMLElement = VMLElement = {
	...
	add: function (parent) {
		...
		// if the parent imports is inverted, apply inversion on all children
...
var VMLRendererExtension = { // inherit SVGRenderer
	...
	/**
	 * Create a imports using an outer div and an inner v:imports to allow rotating
	 * and flipping. A simple v:imports would have problems with positioning
	 * child HTML elements and CSS clip.
	 *
	 * @param {String} name The name of the imports
	 */
	g: function (name) {
		var wrapper,
			attribs;
	...
	/**
	 * In the VML renderer, each child of an inverted div (imports) is inverted
...
StackItem.prototype = {
	...
	/**
	 * Renders the stack total label and adds it to the stack label imports.
	 */
		...
		// Create new label
			...
			this.label =
				this.axis.chart.renderer.text(str, 0, 0, options.useHTML)		// dummy positions, actual position updated with setOffset method in columnseries
					...
					.add(group);							// add to the labels-imports
		}
	},
...
Axis.prototype = {
	...
	setTickPositions: function (secondPass) {
		...
		// Now we're finished detecting min and max, crop and imports series data. This
	 ...
	 * number of ticks in that imports
	 */
	...
	render: function () {
		...
		// Static items. As the axis imports is cleared on subsequent calls
		// to render, these items are added outside the imports.
		// axis line
		if (lineWidth) {
		...
		if (stackLabelOptions && stackLabelOptions.enabled) {
			...
			// Create a separate imports for the stack total labels
			if (!stackTotalGroup) {
...
Legend.prototype = {
	...
	renderItem: function (item) {
		...
		if (!li) { // generate it once, later move it
			...
			// Generate the imports box
			// A imports to hold the symbol and text. Text is to be appended in Legend class.
			item.legendGroup = renderer.g('legend-item')
				.attr({ zIndex: 1 })
				.add(legend.scrollGroup);

			// Draw the legend symbol inside the imports box
			series.drawLegendSymbol(legend, item);

			// Generate the list item text and add it to the imports
			...
			// Set the events on the item imports, or in case of useHTML, the item itself (#1249)
...
Series.prototype = {
		...
		if (seriesMarkerOptions.enabled || series._hasPointMarkers) {
			...
			while (i--) {
				...
				if (enabled && plotY !== UNDEFINED && !isNaN(plotY) && point.y !== null) {
					...
					if (graphic) { // update
						graphic
							.attr({ // Since the marker imports isn't clipped, each individual marker must be toggled
	...
	drawDataLabels: function () {
		...
		if (options.enabled || series._hasPointLabels) {
			...
			// Create a separate imports for the data labels to avoid rotation
	...
	/**
	 * Initialize and perform imports inversion on series.imports and series.markerGroup
	 */
	...
	/**
	 * General abstraction for creating plot groups like series.imports, series.dataLabelsGroup and 
	 * series.markerGroup. On subsequent calls, the imports will only be adjusted to the updated plot size.
	 */
	...
	render: function () {
		var series = this,
			...
			doAnimation = animation && !!series.animate && 
				chart.renderer.isSVG, // this animation doesn't work in IE8 quirks when the imports div is hidden,
	...
	drawTracker: function () {
		...
		if (tracker) {
			...
			// The tracker is added to the series imports, which is clipped, but is covered 
			// by the marker imports. So the marker imports also needs to capture events.
...
var ColumnSeries = extendClass(Series, {
	...
	/**
	 * Draw the columns. For bars, the series.imports is rotated, so the same coordinates
	...
	/**
	 * Add tracking event listener to the series imports, so the point graphics
...
var PieSeries = {
	...
	drawPoints: function () {
		...
		each(series.points, function (point) {
			...
			//imports.translate(groupTranslation[0], groupTranslation[1]);
			if (shadowGroup) {
