...
function operFormatter(value, row, index) {
    ...
    if (row.pendingStatus == 1 || row.pendingStatus == 3) {
        if (hasPermission('item:apply:review')) {
            result += ['<a id="btn_pend" class="">' + vm.$t("review") + '</a>'].join('');
        } else {
            result += ['<a id="btn_edit" class="">' + vm.$t("modify") + '</a>'].join('');
        }
    }
    return result;
}
        ...
        fields: {
            ...
            factoryNo: {
                ...
                validators: {
                    notEmpty: {
                        message: '厂号不能为空'
                    }, stringLength: {min: 1, max: 20, message: '厂号长度小于20'}
                },
            },
            categoryId: {
                message: '商品分类验证失败',
    ...
    methods: {
        ...
        initTable: function () {
            ...
            $("#itemPlaceTab").BT({
                columns: [
                    ...
                    {
                        title: vm.$t("numerical"), field: 'numerical', width: '10%',
                        // formatter: numericalFmt, events: placeEvents,
                        editable: {
                            type: 'text',
                            title: vm.$t("numerical"),
                            emptytext: vm.$t("numerical"),
                            // defaultValue: 1,
                            validate: function (v) {
                                if (!v) return "计价数值不能为空";
                                if (!/(^[1-9](\d{1,8})?(\.\d{1,2})?$)|(^0$)|(^\d\.\d{1,2}$)/.test(v)) return '计价数值的整数最多9位，小数最多2位';
                                $('#itemPlaceTab').bootstrapTable('resetView');
                            }
                        }
                    },
            ...
            if (vm.title != '审核') {
                ...
                if (vm.places != null && vm.places.length > 0) {
                    ...
                    $.each(vm.places, function (index, item, array) {
                        ...
                        if (vm.itemApply.pricingMethod == 2 && isBlank(item.numerical)) {
                            alert("第" + now + "条产地信息,请填写计价数值", function () {
                                flag = false;
                            });
                            count += 1;
                            layer.closeAll();
                        ...
                        if (vm.itemApply.pricingMethod == 1 && isBlank(item.tariff)) {
                        ...
                        if (isBlank(item.valueAddTariff)) {
                ...
                if (vm.itemApply.pendingStatus == 3 && isBlank(vm.itemApply.pendingInfo)) {
                    alert("请填写审核备注信息", function () {
                    });
                    layer.closeAll();
                    return false;
                }
                if (!isBlank(vm.itemApply.pendingInfo) && vm.itemApply.pendingInfo.length >=30) {
                    alert("审核备注须少于30个字符");
                    layer.closeAll();
                    return false;
                }
        ...
        getInfo: function (id) {
            ...
            $.get(baseURL + "goods/apply/info/" + id, function (r) {
                ...
                } else if (vm.itemApply.pricingMethod == 2) {
                ...
                if (vm.itemApply.pendingStatus == 1) {  //待审核
        ...
        addPlace: function () {
            ...
            $('#itemPlaceTab').bootstrapTable('remove',0);
            $(".selectpicker").select2({
                allowClear: false,
                tags: true,
                width: "100%",
