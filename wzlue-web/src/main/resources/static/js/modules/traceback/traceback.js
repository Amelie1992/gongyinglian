...
//报检号追溯 详情
function opFormatter2(value, row, index) {//------------------------------
    var op = '';
    op += '<a id="btn_detail2">' + vm.$t('Details') + '</a>';
    return op;
}
...
window.opEvent = {
    'click #btn_detail': function (event, value, row, index) {
        vm.showList = false;
        vm.detailList = true;
        vm.inspectionNoList = false;
        /*vm.addList = false;
        vm.detailList = true;*/
        vm.getInfo(row.id, vm.params.orderType, row.orderNumber, row.contractNumber);
    },
    //报检号追溯 详情
    'click #btn_detail2': function (event, value, row, index) {
        vm.showList = false;
        vm.detailList = true;
        vm.inspectionNoList = false;
        vm.getInfo(row.id, row.type, row.orderNumber, row.contractNumber);
    }
};//-------------------------------
...
var vm = new Vue({
    ...
    methods: {
        ...
        initTable2: function () {
            //列表
            $("#table2").bootstrapTable("destroy"); //**********对于查询必不可少
            $("#table2").BT({
                url: baseURL + 'traceBack/list2',
                columns: [
                    {checkbox: true},
                    {title: vm.$t("order") + vm.$t("type"), field: 'type', formatter: function (value, row, index) {
                            if (value == 0) {
                                return "货物订单";
                            }else if (value == 1) {
                                return "物流订单";
                    ...
                    {title: vm.$t("InspectionNo"), field: 'inspectionNo'},
                    {title: vm.$t("OrderNumber"), field: 'orderNumber'},
                    {title: vm.$t("ContractNumber"), field: 'contractNumber'},
                    {title: vm.$t("NameOfSellerCompany"), field: 'merchantCopName'},
                    {title: vm.$t("NameOfBuyerCompany"), field: 'customsCopName'},
                    {title: vm.$t("Founder"), field: 'createByName'},
                    {title: vm.$t("CreationTime"), field: 'createTime'},
                    {title: vm.$t('chaozuo'), formatter: opFormatter2, events: opEvent}
                ],
                //条件查询
        ...
        //报检号追溯
        traceBack: function (val) {
            if (val == 1) {//报检号追溯
                vm.showList = false;
                vm.detailList = false;
                vm.inspectionNoList = true;
                vm.initTable2();
