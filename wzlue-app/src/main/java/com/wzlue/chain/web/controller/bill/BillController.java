            ...
            if (storageOrderCostId != null) {
                ...
                if (orderCost != null) {
                    if (StringUtils.isNotBlank(orderCost.getMoney16())){
                        List<FeeObj> feeList = net.sf.json.JSONArray.toList(JSONArray.fromObject(orderCost.getMoney16()), FeeObj.class);
...
//        return R.ok().put("map", map);
    ...
    public R gatheringMethod(@RequestBody GatheringVO gatheringVO) {
        ...
        Integer billId = paymentRecords.get(0).getBillId();
        BillEntity bill = billService.queryObject(billId);
        if (bill.getOrderType() == 3 && bill.getReceivableStatus() == 1) { //仓储订单款项已清
            OrderEntity storageOrder = orderService.queryByOrderNumber(bill.getOrderNumber());
            if (storageOrder.getPassStatus()==6){ //仓储订单货物已全部出库
                storageOrder.setPayStatus(1);
                storageOrder.setPassStatus(7);
                orderService.update(storageOrder);
                //订单交易完成---增加双方信用分数
                MerchantCompanyEntity company1 = merchantCompanyService.queryObject(bill.getPayerId());
                MerchantCompanyEntity company2 = merchantCompanyService.queryObject(bill.getPayeeId());
                ...
                //设置订单合同到期日期
                storageContractService.setExpireDateByOrderNumber(bill.getOrderNumber());
            }
        }
