       ...
       /* if (StringUtils.isBlank(agentOrder.getContractNumber())) {
        ...
        if (agentOrder.getGoods().size() > 0 && agentOrder.getGoods().get(0) != null) {
            for (AgentOrderGoodsEntity item : agentOrder.getGoods()) {
                ...
                if(!"1".equals(goodsUnit) && !"2".equals(goodsUnit) && !"3".equals(goodsUnit)&& !"4".equals(goodsUnit))
...
*/
            ...
            if (null != offerEntity) {
                ...
                if (offerEntity.getTransactionNumber()==null) {
                    offerEntity.setTransactionNumber(1);
                }else {
                    offerEntity.setTransactionNumber(offerEntity.getTransactionNumber()+1);
                }
        ...
        /*    }*/
            ...
            cuscontractEntity.setSigningTime(now);
            mercontractEntity.setSigningTime(now);
...
//	    flag 判断分支 4则为 取消订单 5则为 系统挂起订单 6则为 删除订单 7则为 买家取消 8则为买家取消
        if (flag == 6) {
            ...
            //成交数量-1
           /* AgentOrderEntity agentOrder = agentOrderDao.queryObject(id);
            AgentOfferEntity offerEntity = agentOfferDao.queryInfo(agentOrder.getOfferId());
            if (null != offerEntity) {
                if (offerEntity.getTransactionNumber()==null || offerEntity.getTransactionNumber()==0) {
                    offerEntity.setTransactionNumber(0);
                }else {
                    offerEntity.setTransactionNumber(offerEntity.getTransactionNumber()-1);
                }
                agentOfferDao.update(offerEntity);
