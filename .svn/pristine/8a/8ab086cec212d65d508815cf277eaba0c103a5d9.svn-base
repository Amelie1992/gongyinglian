            ...
            amountCollected: {
                ...
                validators: {
                    notEmpty: {
                        ...
                        //|(^0$)
                        regexp: /(^[1-9](\d{1,17})?(\.\d{1,2})?$)|(^\d\.\d{1,2}$)/,
                        message: '实收金额为整数位最多18位，可保留2位小数的正数'
                    }
                }
            }
        }
        ...
        initTable: function () {
            ...
            $("#table").BT({
                ...
                columns: [
                    ...
                    {
                        title: vm.$t("AmountReceivable"), field: 'amountReceivable2',
                    ...
                    {
                        title: vm.$t("AmountReceived"), field: 'amountReceived2',
                        formatter: function (value, row, index) {
                            var str = value == null ? '0' : value;
                            if (row.unit === 0) {
                                return str + '&nbsp;元';
                    ...
                    {
                        title: vm.$t("SurplusAmount"), field: 'remainingAmountReceivable2',
                        formatter: function (value, row, index) {
                            var str = value == null ? '0' : value;
                            if (row.unit === 0) {
                                return str + '&nbsp;元';
        ...
        showGatheringModal: function (event) {
            ...
            if (vm.paymentRecords.length == rows.length) {
                vm.$nextTick(function () {
                    vm.gathering.unit = vm.paymentRecords[0].unit;
                });
