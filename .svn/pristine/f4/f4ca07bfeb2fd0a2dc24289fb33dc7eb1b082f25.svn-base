...
//        ConcurrentModificationException
        ArrayList<MerchantCompanyEntity> removeList = new ArrayList<>();
        ArrayList<MerchantCompanyEntity> addList = new ArrayList<>();
        for (MerchantCompanyEntity company : merchantUsersList) {
            if (company.getAuditStatus().equals("0") && StringUtils.isNotBlank(company.getModifyMsg())) { //修改待审核
                MerchantCompanyEntity companyEntity = JSON.parseObject(company.getModifyMsg(), MerchantCompanyEntity.class);
//                merchantUsersList.remove(company);
//                merchantUsersList.add(companyEntity);
                removeList.add(company);
                addList.add(companyEntity);
            }
        }
        merchantUsersList.removeAll(removeList);
//        merchantUsersList.addAll(addList);
        addList.addAll(merchantUsersList);//待审核优先排序

        return R.page(addList, total);
    }
...
//    @RequiresPermissions("company:merchantusers:info")
    public R info(@PathVariable("id") Integer id) {
        ...
        if (merchantUsers.getAuditStatus().equals("0") && StringUtils.isNotBlank(merchantUsers.getModifyMsg())) {
            merchantUsers = JSON.parseObject(merchantUsers.getModifyMsg(), MerchantCompanyEntity.class);
        }
...
//    @RequiresPermissions("company:merchantusers:update")
    public R auditpass(@RequestBody Map<String, Object> param) {
        ...
        } else if (StringUtils.isNotBlank(company.getModifyMsg())) {
            company = JSON.parseObject(company.getModifyMsg(), MerchantCompanyEntity.class);
            company.setModifyMsg("");
//            company.setAuditStatus("1");
            merchantCompanyService.update(company);
            Long userId = company.getUserId();

            // 更新服务类型
            merchantCompanyService.updateServiceType(company);
            // 先删除 再保存
            imageDao.deleteByCreateByAndCode(userId + "");
            List<ImageEntity> imgList;
            if ((imgList = company.getImageEntityList()) != null || imgList.size() > 0) {
...
//    @RequiresPermissions("company:merchantusers:update")
    public R noauditpass(@RequestBody Map<String, Object> param) {
        MerchantCompanyEntity company = new MerchantCompanyEntity() {{
            setId(Long.valueOf(Long.valueOf(param.get("id").toString())));
        }};
        company.setModifyMsg("");
        merchantCompanyService.update(company);
        merchantCompanyService.updateCreditType(CreditType.AUDITFAILED, company);
...
//            chatMessageService.createNewIMUser(user.getUsername());
     ...
     * 获取用户的用户信息. 所属企业信息2
     */
    @RequestMapping("/getUserCompany2")
    public R getUserCompany2() {

        Long userid;
        if (null == (userid = getUserId())) return R.error("登陆失效");

        SysUserEntity userEntity = sysUserService.queryObject(userid);
        List<MerchantCompanyEntity> companies = merchantCompanyService.queryByUserId2(userid);

        Map<String, Object> allMap = new HashMap<>();
        allMap.put("companies", companies);
        allMap.put("user", userEntity);
