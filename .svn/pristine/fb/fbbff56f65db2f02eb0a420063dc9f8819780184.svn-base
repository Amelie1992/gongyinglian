...
//        declareOrder.setUnit(Long.valueOf(offer.getUnit()));
    ...
    public void update(DeclareOrderEntity declareOrder) {
        ...
        if (declareOrder.getOrderState() == 1) {//已接单
       ...
       /* if (declareOrder.getOrderState() == 7) {//订单已取消
            //订单取消，并对出售的报盘产品进行自增
            DeclareOrderEntity declareOrderEntity = declareOrderDao.queryObject(declareOrder.getId());
...
//          Long id = (Long) map.get("id"); //Integer cannot be cast to Long
        Object idI = map.get("id");
        Long id = Long.valueOf(idI.toString());
//        if (orderState != null && orderState == 7) {//取消订单 成交数-1
//            declareOrderDao.updateByOrderIdLess(Long.valueOf(id.toString()));
//        }
        if (orderState != null && orderState == 6) {//确认收货--订单完结
            // 设置订单合同到期日期
            declareContractDao.setExpireDateByOrderId(id);
            // 加信用分数
            DeclareOrderEntity declareOrder = declareOrderDao.queryObject(id);
            MerchantCompanyEntity buyer = merchantCompanyDao.queryObject(declareOrder.getBuyCompanyId());
            MerchantCompanyEntity seller = merchantCompanyDao.queryObject(declareOrder.getMerchantCompanyId());
            merchantCompanyDao.updateCreditType(CreditType.COMPLETEORDER, buyer);
            merchantCompanyDao.updateCreditType(CreditType.COMPLETEORDER, seller);
        }
    }
